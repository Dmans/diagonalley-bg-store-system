<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml" ng-app='employApps'>
	<head>
		...
		<script type="text/javascript" src="<?=base_url(); ?>scripts/angular.min.js"></script>
		<script type="text/javascript" src="<?=base_url(); ?>scripts/apps/employ/employController.js"></script>
	...
	<body ng-controller='employListCtrl'>
		<h3>個人打卡鐘</h3>
		<div>
			<div>打卡使用者:<span style="font-weight: bold;">{{data.usr_name}}</span></div>
			<div>打卡店鋪:<span style="font-weight: bold;">{{data.store.sto_name}}</span></div>
		</div>
		<div>
		...
		<div class="span9">本月打卡紀錄
			<table class="table table-striped table-hover table-bordered table-condensed">
				...
				<tr ng-repeat="(key, value) in data.current_check" id="chkeckTr_{{value.chk_num}}">
					<td>{{key+1}}</td>
					<td>{{value.chk_in_time}}</td>
					<td style="font-weight: bold;">
						<div ng-if="value.chk_out_time === null">
							<input type="button" id="checkoutButton" value="下班打卡" class="btn btn-success"/>
							<input type="hidden" id="chkNum" value="{{value.chk_num}}" />
						</div>
						<div ng-if="value.chk_out_time !== null">{{value.chk_out_time}}</div>
					</td>
					<td><div>{{value.confirm_usr_num !== null ? "已審核":"未審核"}}</div></td>
					<td><div ng-show="value.confirm_date !== null">{{value.confirm_date}}</div></td>
					<td><div ng-show="value.confirm_hours !== null">{{value.confirm_hours}}</div></td>
				</tr>
			</table>
		</div>
