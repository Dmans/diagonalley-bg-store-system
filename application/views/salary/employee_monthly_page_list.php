...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body class="container-fluid">
		<h3>每月正職人員薪資審核</h3>
		<div>
			<? echo form_open('salary/salary_action/employee_monthly_list'); ?>
				<div>審核月份:
		...
		<div>
			<? if(isset($query_result)):  ?>
				<? echo form_open('salary/salary_action/employee_salary_confirm', 'id="mainForm"'); ?>
				<div>
					<h3><?php echo $year_month?> 正職人員薪資審核</h3>
					...
					<div>
						<table id="table_<?php echo $chk_user->usr_num ?>" class="table table-striped table-hover table-bordered table-condensed">
							...
							<tr>
								...
								<td></td>
								<td></td>
								<td align="right">基本工時(B)</td>
								<td></td>
								<td><span id="usrBaseHours_<?php echo $chk_user->usr_num ?>" usrnum="<?php echo $chk_user->usr_num ?>"><?php echo $chk_user->usr_base_hours ?></span></td>
							</tr>
							<tr>
								<td></td>
								<td></td>
								<td></td>
								<td align="right">前期累計積假(C)</td>
								<td></td>
								<td><span id="accumulateLeave_<?php echo $chk_user->usr_num ?>" usrnum="<?php echo $chk_user->usr_num ?>"><?php echo $chk_user->previous_say_leave_balance?></span></td>
							</tr>
							...
							<tr>
								...
								<td></td>
								<td></td>
								<td align="right">本月積假結算(A)-(B)+(C)</td>
								<td></td>
								<td><span id="leaveBalance_<?php echo $chk_user->usr_num ?>" usrnum="<?php echo $chk_user->usr_num ?>"><?php echo $chk_user->say_leave_balance?></span></td>
							</tr>
						</table>
						<div id="extraOptions_<?php echo $chk_user->usr_num ?>" class="list-group"></div>
						<div>
			...
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					<div class="modal-header">
						<h4 class="modal-title" id="myModalLabel"><?php echo $year_month?> 正職人員薪資審核送出確認</h4>
					</div>
